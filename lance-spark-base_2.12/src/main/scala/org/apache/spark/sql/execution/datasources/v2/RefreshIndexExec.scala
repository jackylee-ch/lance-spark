...
case class RefreshIndexExec(
  ...
  private def planRefresh(ds: Dataset, readOptions: LanceSparkReadOptions): RefreshPlan = {
    ...
    if (segments.exists(!_.fragments().isPresent)) {
      throw new UnsupportedOperationException(
        s"Index '$indexName' has segments that predate fragment coverage tracking, so its " +
          "coverage cannot be determined. Rebuild it with ALTER TABLE ... CREATE INDEX.")
    }
