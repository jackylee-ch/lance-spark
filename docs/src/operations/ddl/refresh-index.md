...
[OPTIMIZE INDEX](./optimize-index.md) fills the same missing coverage but runs on the Spark driver
rather than distributing the build, and can also merge existing segments. Use `REFRESH INDEX` to
parallelize the build across executors; use `OPTIMIZE INDEX` for driver-side maintenance or to merge
segments after either command.
...
Whether an index needs this depends on what it stores; see [Compaction](#notes-and-limitations) below
for which methods keep coverage after a rewrite.
...
- **Method Options Are Not Inherited**: see [Index Method Options](#index-method-options).
- **Indexes Created Before Coverage Tracking**: a segment that predates fragment coverage tracking
  has no fragment bitmap, so its coverage cannot be determined. `REFRESH INDEX` rejects such an
  index rather than silently rebuilding the whole table; rebuild it with `CREATE INDEX`.
...
- **Concurrent `REFRESH INDEX`**: do not run two refreshes of the same index concurrently. The second
  will either fail with a version conflict or succeed and add duplicate segment coverage over the same
  fragments. `SHOW INDEXES` makes duplicate coverage visible via `num_segments`; merge segments with
  [OPTIMIZE INDEX](./optimize-index.md) or rebuild with [CREATE INDEX](./create-index.md).
...
- **Segment Growth**: each refresh adds segments to the index, and Lance can only compact fragments
  ...
  after, and merge segments with [OPTIMIZE INDEX](./optimize-index.md) or rebuild with
  [CREATE INDEX](./create-index.md) when the count grows. [SHOW INDEXES](./show-indexes.md) reports
  `num_segments`.

## See Also

- [CREATE INDEX](./create-index.md)
- [OPTIMIZE INDEX](./optimize-index.md)
